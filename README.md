...
1. 利用者にすぐ回避してもらうなら、URL に `?nosw` を付けて開いてもらう
   ...
   全削除**を行い、以後は Service Worker の居ない素のサイトに戻る。離脱は端末に残るので、
   次に普通の URL で開いても登録し直さない。**戻すときは `?sw` を付けて開く**。
...
2. 全端末から取り消すなら、`scripts/sw-kill.js` の中身で `public/sw.js` を上書きして push する。
   （`sw-kill.js` は焼き込みの口を持っているのでそのままビルドが通る。回復手段が
   ビルドで落ちると、まさに要る場面で配れない。）
